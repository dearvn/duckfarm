...
namespace App\Filament\App\Resources\Shield;

use BezhanSalleh\FilamentShield\Contracts\HasShieldPermissions;
use BezhanSalleh\FilamentShield\Facades\FilamentShield;
use App\Filament\App\Resources\Shield\RoleResource\Pages;
use App\Models\Role;
...
class RoleResource extends Resource implements HasShieldPermissions
{
    ...
    {
        return __('common.resource.roles');
    }
    ...
    {
        return strtolower(__('common.resource.roles'));
    }
        ...
        return $form
            ->schema([
                        ...
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\TextInput::make('name')
                                    ->label(__('role.field.name'))
                                    ->required()
                                    ->maxLength(255),
                                Forms\Components\TextInput::make('guard_name')
                                    ->label(__('role.field.guard_name'))
                                ...
                                Forms\Components\Toggle::make('select_all')
                                    ...
                                    ->label(__('role.field.select_all.name'))
                                    ->helperText(fn (): HtmlString => new HtmlString(__('role.field.select_all.message')))
                                    ->live()
                ...
                Forms\Components\Tabs::make('Permissions')
                    ->contained()
                    ->tabs([
                        Forms\Components\Tabs\Tab::make(__('role.resources'))
                        ...
                        Forms\Components\Tabs\Tab::make(__('role.pages'))
                        ...
                        Forms\Components\Tabs\Tab::make(__('role.widgets'))
                        ...
                        Forms\Components\Tabs\Tab::make(__('role.custom'))
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('name')
                    ->badge()
                    ->label(__('role.column.name'))
                    ->formatStateUsing(fn ($state): string => Str::headline($state))
                    ->colors(['primary'])
                    ->searchable(),
                Tables\Columns\TextColumn::make('guard_name')
                    ->badge()
                    ->label(__('role.column.guard_name')),
                Tables\Columns\TextColumn::make('permissions_count')
                    ->badge()
                    ->label(__('role.column.permissions'))
                    ->counts('permissions')
                    ->colors(['success']),
                Tables\Columns\TextColumn::make('updated_at')
                    ->label(__('role.column.updated_at'))
                    ->dateTime(),
            ])
            ->filters([
                //
